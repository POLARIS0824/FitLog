...
class AgentEngineImpl @Inject constructor(
    ...
    private suspend fun buildCoachInstruction(): String = buildString {
        ...
        appendLine("- 涉及修改数据的操作（记体重、切换计划、创建计划）会弹确认，用户拒绝则不要执行")
        appendLine("- 计划优先：用户有今日计划课次时，建议围绕该课次展开")
        appendLine("- 用户想换/要新计划时，用 createPlan 工具把方案落成系统计划（训练日 JSON 中 exerciseKey 必须来自动作库，可用 searchExercises 查询）；用户认可当前计划调整即可时不必重建")
